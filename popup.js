(function(window) {
  const PngToJpgConvertor = (() => {
    function convertor(imageFileBlob, options) {
      options = options || {};
      const defaults = {
        downloadLinkSelector: '.js-download-jpg'
      };
      ...
      function updateDownloadLink(pngFileName, jpgBlob) {
        ...
        linkEl.setAttribute('download', jpgFileName);
        linkEl.href = window.URL.createObjectURL(jpgBlob);
      ...
      function process() {
        ...
        imageEl.onload = (e) => {
          ...
          canvas.toBlob(updateDownloadLink.bind(window, imageFileBlob.name), 'image/jpe?g', 1);
        };
  ...
  imageFileEl.addEventListener('change', (event) => {
    ...
    if (pngImageFileBlob.type.match(/image\/png/i) !== null) {
      PngToJpgConvertor(pngImageFileBlob).process();
    } else {
      alert('Invalid PNG image file');
    }
  }, false);
})(window);